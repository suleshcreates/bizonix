        ...
        <div className={styles.outcomes__visualStep} key={label}>
          <span className={styles.outcomes__visualIcon}>
            {index === visual.steps.length - 1 ? (
              ...
              <span className={styles.outcomes__miniGlyph}>
          ...
          <span>{label}</span>
          {index < visual.steps.length - 1 ? (
            <i className={styles.outcomes__connector}>
              <b />
            </i>
          ) : null}
        </div>
...
export function OutcomeHeader({ data }: { data: OutcomesSectionData }) {
  return (
    <header className={styles.outcomes__header} data-outcome-header>
      <p className={styles.outcomes__eyebrow}>
        <span aria-hidden="true" />
        {data.eyebrow}
      </p>
      <ModuleHeading
        id="module-outcomes"
        text={data.title + " " + data.highlight}
        accent={data.highlight}
        reveal={false}
      />
      {data.intro ? (
        <p className={styles.outcomes__intro}>{data.intro}</p>
      ) : null}
    </header>
  );
}
...
export function OutcomeRow({ outcome }: { outcome: ModuleOutcome }) {
  ...
  return (
    <article className={styles.outcomes__row} data-outcome-row>
      ...
      <span className={styles.outcomes__iconDisc} aria-hidden="true">
        <Icon />
      </span>
      <div className={styles.outcomes__copy}>
        <h3>{outcome.title}</h3>
...
export function ModuleOutcomes({ data }: { data: OutcomesSectionData }) {
  ...
  return (
    <section
      ref={root}
      className={styles.outcomes__section}
      aria-labelledby="module-outcomes"
      style={{ "--outcome-accent": data.outcomes[0]?.accent } as CSSProperties}
    >
      <div className={styles.outcomes__shell}>
        <OutcomeHeader data={data} />

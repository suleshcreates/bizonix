...
function RecordStream() {
  return (
    <svg
      className={styles.fourPillarsRecord__streamSvg}
      viewBox="0 0 620 300"
    ...
    >
      ...
      {streamGeometry.map((stream) => (
        <path
          ...
          className={styles.fourPillarsRecord__streamPath}
          d={stream.d}
          stroke={`url(#bz-stream-${stream.id})`}
        />
      ))}

      <g className={styles.fourPillarsRecord__streamParticles}>
        {streamGeometry.map((stream) => (
          <circle
            key={stream.id}
            r="3.4"
...
function RecordStreamVertical() {
  return (
    <svg
      className={styles.fourPillarsRecord__streamSvgVertical}
      viewBox="0 0 800 128"
      preserveAspectRatio="none"
      aria-hidden="true"
    >
...
export function RecordConvergence() {
  return (
    <div className={styles.fourPillarsRecord__canvas}>
      <div className={styles.fourPillarsRecord__canvasGrid} aria-hidden="true" />
      <div className={styles.fourPillarsRecord__atmosphere} aria-hidden="true" />

      <header className={styles.fourPillarsRecord__canvasHead}>
        <h3 className={styles.fourPillarsRecord__canvasTitle}>One record. Many pillars.</h3>
      </header>

      <div className={styles.fourPillarsRecord__canvasBody}>
        <ul className={styles.fourPillarsRecord__sourceList}>
          {pillars.map((pillar) => {
            const Icon = sourceIcon[pillar.id];
            return (
              <li key={pillar.id} className={styles.fourPillarsRecord__sourceNode} data-accent={pillar.accent}>
                <span className={styles.fourPillarsRecord__sourceIcon}>
                  <Icon size={15} strokeWidth={2.1} aria-hidden="true" />
                </span>
                <span className={styles.fourPillarsRecord__sourceText}>
        ...
        <div className={styles.fourPillarsRecord__streamField}>
          <RecordStream />
          <RecordStreamVertical />
        </div>

        <div className={styles.fourPillarsRecord__recordZone}>
          <RecordObject />
          <RecordExplanation />
        </div>
      </div>

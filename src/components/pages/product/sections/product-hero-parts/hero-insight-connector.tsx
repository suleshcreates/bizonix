...
export function HeroInsightConnector({ side }: HeroInsightConnectorProps) {
  return (
    <svg
      className={`${styles.productHero__connectors} ${side === "left" ? styles.productHero__connectorLeft : styles.productHero__connectorRight}`}
      width="132"
      height="96"
      viewBox="0 0 132 96"
      fill="none"
    ...
    >
      <path
        className={styles.productHero__connectorPath}
        d="M9 9C9 40 21 58 52 68C81 77 104 82 128 86"
      />
      <circle className={styles.productHero__connectorHalo} cx="9" cy="9" r="6.5" />
      <circle className={styles.productHero__connectorNode} cx="9" cy="9" r="2.6" />
    </svg>
  );
}

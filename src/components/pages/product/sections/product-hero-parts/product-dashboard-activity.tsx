import {
  ArrowLeftRight,
  Package,
  Receipt,
  RotateCcw,
  ShoppingBag,
} from "lucide-react";
import { heroActivity } from "@/lib/content/product/product-hero-demo-data";
import styles from "@/components/pages/product/product.module.css";

const toneIcon = {
  sale: ShoppingBag,
  move: ArrowLeftRight,
  money: Receipt,
  inward: Package,
  return: RotateCcw,
} as const;

export function ProductDashboardActivity() {
  return (
    <section className={`${styles.productHeroDashboard__card} ${styles.productHeroDashboard__activityCard}`}>
      <header className={styles.productHeroDashboard__cardHead}>
        <p>Real-time activity</p>
        <small>live feed</small>
      </header>

      <div className={styles.productHeroDashboard__activityList}>
        {heroActivity.map((item) => {
          const Icon = toneIcon[item.tone];
          return (
            <div key={item.ref} className={styles.productHeroDashboard__activityItem}>
              <span
                className={styles.productHeroDashboard__activityDot}
                data-tone={item.tone}
                aria-hidden="true"
              >
                <Icon size={12} strokeWidth={2.1} />
              </span>
              <span className={styles.productHeroDashboard__activityBody}>
                <strong>{item.title}</strong>
                <span>
                  {item.ref} · {item.time} ago
                </span>
              </span>
            </div>
          );
        })}

        <div className={styles.productHeroDashboard__activityFoot}>
          View full audit trail
          <span>18 events today</span>
        </div>
      </div>
    </section>
  );
}

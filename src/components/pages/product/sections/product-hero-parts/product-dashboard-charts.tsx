...
export function ProductDashboardSalesTrend() {
  ...
  return (
    <section className={`${styles.productHeroDashboard__card} ${styles.productHeroDashboard__trendCard}`}>
      ...
      <div className={styles.productHeroDashboard__trendTop}>
        <span className={styles.productHeroDashboard__trendTotal}>{heroSalesTrendAxis.total}</span>
        <span className={styles.productHeroDashboard__trendMeta}>
          <span className={`${styles.productHeroDashboard__kpiDelta} ${styles.productHeroDashboard__deltaUp}`}>
      ...
      <div className={styles.productHeroDashboard__trendChart}>
        <svg
        ...
        >
          ...
          {[TOP, 34, 63, BASELINE].map((y) => (
            <line
              key={y}
              className={styles.productHeroDashboard__gridLine}
              x1="0"
              y1={y}
              x2={VIEW_W}
              y2={y}
          ...
          {heroSalesTrend.map((value, index) => (
            <rect
              key={index}
              className={
                value === peak ? styles.productHeroDashboard__trendBarPeak : styles.productHeroDashboard__trendBar
              }
              x={barX(index)}
              y={barY(value)}
              width={BAR_W}
          ...
          <path className={styles.productHeroDashboard__trendLine} d={smoothPath(linePoints)} />
        </svg>
      </div>

      <footer className={styles.productHeroDashboard__trendAxis}>
...
export function ProductDashboardBranchSales() {
  return (
    <section className={`${styles.productHeroDashboard__card} ${styles.productHeroDashboard__branchCard}`}>
      <header className={styles.productHeroDashboard__cardHead}>
        <span className={styles.productHeroDashboard__cardIcon} aria-hidden="true">
      ...
      <div className={styles.productHeroDashboard__branchList}>
        {heroBranchSales.map((branch, index) => (
          <div key={branch.name} className={styles.productHeroDashboard__branchRow}>
            <div className={styles.productHeroDashboard__branchTop}>
              <span>{branch.name}</span>
              <strong>{branch.value}</strong>
            </div>
            <div className={styles.productHeroDashboard__branchTrack}>
              <i
                className={styles.productHeroDashboard__branchFill}
                data-rank={index}

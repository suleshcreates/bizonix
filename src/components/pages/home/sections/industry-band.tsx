          ...
          {industryConfig.map((item, i) => {
            ...
            return (
              <Link
                key={item.title}
                href={item.href}
                className="group flex flex-col justify-between overflow-hidden rounded-[24px] border border-bz-border/70 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-bz-blue/30 hover:shadow-soft sm:rounded-[32px]"
              >
                {/* Header Graphic Area */}
                <div
                  className={`blue-grid relative flex h-44 items-center justify-center overflow-hidden bg-gradient-to-br sm:h-48 ${item.gradient}`}
                >
                  <VisualComponent />
                  {/* The inner visual fills 85% of the frame, so on a narrow
                      card the index sat underneath it; a frosted chip lifts
                      it clear without moving it out of the corner. */}
                  <span className="absolute left-4 top-4 z-10 rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-[.16em] text-bz-blue/70 backdrop-blur-sm transition-colors duration-300 group-hover:text-bz-blue sm:left-5 sm:top-5">
                    0{i + 1}
                  </span>
                </div>
                ...
                <div className="flex flex-grow flex-col justify-between p-6 sm:p-8">
                  <div>
                    ...
                    <p className="mt-2.5 text-sm leading-relaxed text-bz-muted">
                      {item.body}
                    </p>
                  </div>
                  <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-bz-blue transition-colors group-hover:text-bz-blue-hover sm:mt-6">
                    Explore industry{" "}
                    <ArrowRight
                      size={16}

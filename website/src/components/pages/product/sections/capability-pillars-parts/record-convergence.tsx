...
import {
  ...
  type PillarId,
} from "@/lib/content/product/four-pillars-data";
...
function DataStream() {
  return (
    <svg
      className={styles.capabilityPillarsGrid__streamSvg}
      viewBox="0 0 764 300"
    ...
    >
      ...
      {streamGeometry.map((stream) => (
        <path
          ...
          className={styles.capabilityPillarsGrid__streamPath}
          d={stream.d}
          stroke={`url(#bz-stream-${stream.id})`}
        />
      ))}

      <g className={styles.capabilityPillarsGrid__streamParticles}>
        {streamGeometry.map((stream) => (
          <circle
            key={stream.id}
            r="3.4"
...
function DataStreamVertical() {
  return (
    <svg
      className={styles.capabilityPillarsGrid__streamSvgVertical}
      viewBox="0 0 800 128"
      preserveAspectRatio="none"
      aria-hidden="true"
    >
...
export function RecordConvergence() {
  return (
    <div className={styles.capabilityPillarsGrid__canvas}>
      <div className={styles.capabilityPillarsGrid__canvasGrid} aria-hidden="true" />

      <header className={styles.capabilityPillarsGrid__canvasHead}>
        <h3 className={styles.capabilityPillarsGrid__canvasTitle}>Four streams. One record.</h3>
        <span className={styles.capabilityPillarsGrid__canvasNote}>
          Independent pillars, one shared operating truth
        </span>
      </header>

      <div className={styles.capabilityPillarsGrid__canvasBody}>
        <ul className={styles.capabilityPillarsGrid__sourceList}>
          {pillars.map((pillar) => {
            const Icon = sourceIcon[pillar.id];
            return (
              <li key={pillar.id} className={styles.capabilityPillarsGrid__sourceNode} data-accent={pillar.accent}>
                <span className={styles.capabilityPillarsGrid__sourceIcon}>
                  <Icon size={15} strokeWidth={2.1} aria-hidden="true" />
                </span>
                <span className={styles.capabilityPillarsGrid__sourceText}>
        ...
        <div className={styles.capabilityPillarsGrid__streamField}>
          <DataStream />
          <DataStreamVertical />
        </div>

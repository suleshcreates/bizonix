"use client";
import { Network, Store, TrendingUp } from "lucide-react";
...
export function ProductStage({ visualVariant = 'dashboard' }: { visualVariant?: string }) {
        ...
        <div ref={breatheRef} className={styles.productStage__breathe}>
          <div className={styles.productStage__screens}>
            {visualVariant !== 'dashboard' ? (
              <div className="w-full h-full flex flex-col items-center justify-center bg-slate-900/60 rounded-xl border border-slate-700/50 backdrop-blur-md relative overflow-hidden" style={{ minHeight: '500px', aspectRatio: '16/9' }}>
                <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 to-teal-500/10" />
                <div className="flex flex-col items-center justify-center text-slate-300 gap-6 z-10 p-12">
                  {visualVariant === 'network' && <Network className="w-24 h-24 text-teal-400 opacity-80" strokeWidth={1} />}
                  {visualVariant === 'storefront' && <Store className="w-24 h-24 text-blue-400 opacity-80" strokeWidth={1} />}
                  {visualVariant === 'metrics' && <TrendingUp className="w-24 h-24 text-indigo-400 opacity-80" strokeWidth={1} />}
                  <div className="text-2xl font-light tracking-wide capitalize text-white">
                    {visualVariant} Module
                  </div>
                  <div className="text-sm text-slate-400 max-w-md text-center leading-relaxed">
                    Visual representation for this architectural variant. In production, this would be a specialized 3D GSAP component similar to the main operating dashboard.
                  </div>
                </div>
              </div>
            ) : (

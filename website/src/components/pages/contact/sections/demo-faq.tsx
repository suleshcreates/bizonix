...
export function DemoFaq() {
  ...
  const [faqItems, setFaqItems] = useState<readonly FaqItem[] | FaqItem[]>(demoFaq);
  const base = useId();
  const { ref, revealed } = useReveal<HTMLElement>();

  useEffect(() => {
    const fetchLiveDemoFaqs = async () => {
      try {
        const apiBase = (process.env.NEXT_PUBLIC_API_URL || "http://localhost:3001/api/v1").replace(/\/$/, "");
        const res = await fetch(`${apiBase}/public/faqs/book-demo`, { cache: "no-store" });
        if (res.ok) {
        ...
        // Fallback silently to static content
      }
    };
    fetchLiveDemoFaqs();
  }, []);
    ...
    >
      <div className={styles.contact__faqIntro}>
        ...
        <div className={styles.contact__faqSignal} aria-hidden="true">
          <span>{String(faqItems.length).padStart(2, "0")}</span>
          <div>

...
| `/api/demo-request` | Contact form endpoint | Connected to backend | Validates and forwards each request to the backend; PostgreSQL and Resend delivery are configured only in the backend environment. |
...
- **Status:** BUILT & REAL
- **What content currently exists:** The form captures complete operating context, consent, and a hidden honeypot. It validates locally, then submits to `/api/demo-request`; the backend persists the request before using Resend for confirmation and internal notification.
- **Where content is guessed:** The success copy repeats the unverified one-business-day response promise. “Calendar booking will be added in Phase 3” is a roadmap statement. Actual lead delivery requires the backend PostgreSQL and Resend environment configuration.
- **Specific information needed to make this real:**
  - Confirm required/optional lead fields, privacy/consent wording, data controller, retention, and CRM/email routing.
  - Provide verified backend Resend sender/recipient details and determine whether Calendly is an approved roadmap claim.
